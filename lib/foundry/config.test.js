...
const WORK_ROOT = resolve(import.meta.dirname, '.config-test-work');
let tempCounter = 0;

/** Create a clean temp dir with a .secops subdirectory ready to receive foundry.yaml */
function makeTempRoot() {
  const root = join(WORK_ROOT, `case-${++tempCounter}`);
  rmSync(root, { recursive: true, force: true });
  mkdirSync(join(root, '.secops'), { recursive: true });
  return root;
}
...
describe('loadFoundryConfig — missing config', () => {
  it('returns null when .secops/foundry.yaml does not exist', () => {
    const tmpRoot = makeTempRoot();
    try {
      rmSync(join(tmpRoot, '.secops'), { recursive: true, force: true });
      const result = loadFoundryConfig(tmpRoot);
