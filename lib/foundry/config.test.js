...
describe('loadFoundryConfig — fixture contract matrix', () => {
  const fixtures = [
    ...
    { name: 'partial-foundry.yaml',              file: 'partial-foundry.yaml',              expectNull: false },
...
// ---------------------------------------------------------------------------
// 7. SAFETY SMELL FINDING — partial fixture (missing endpoint)
// ---------------------------------------------------------------------------
// ⚠️ REVIEWER FINDING: partial-foundry.yaml (no endpoint field) returns non-null from
// loadFoundryConfig(). This is FAIL-OPEN behavior: the config is considered valid even
// though it cannot produce a working URL. resolveEndpoint() will silently build a
// relative URL like "/anthropic/v1/messages" — a broken request that will fail at
// runtime with a confusing error rather than at load time.
//
// RECOMMENDATION (Phase 1): Add endpoint presence validation to loadFoundryConfig()
// so that a missing/empty endpoint returns null (fail-closed). This is a safety issue,
// not just a cosmetic one: a CI environment might pass loadFoundryConfig() checks but
// then fail in production with a mysterious 404 or ENOTFOUND.
// ---------------------------------------------------------------------------

describe('loadFoundryConfig — partial fixture safety smell (DOCUMENTED, NOT blocking)', () => {
  let tmpRoot;
  ...
  it('[CURRENT BEHAVIOR] partial config (no endpoint) returns non-null — fail-open', () => {
    // This documents the current behavior per Sydnor's stated contract.
    // ⚠️ Phase 1 recommendation: this should return null (fail-closed) because
    // resolveEndpoint() will produce a broken relative URL without a base endpoint.
    const result = loadFoundryConfig(tmpRoot);
    assert.notEqual(result, null, 'Current behavior: partial config is accepted (fail-open)');
    // Document that endpoint is indeed missing
    assert.equal(result.foundry.endpoint, undefined, 'Endpoint must be absent in this fixture');
  });

  it('[SAFETY SMELL] resolveEndpoint with missing endpoint produces broken relative URL', () => {
    const result = loadFoundryConfig(tmpRoot);
    const activeDep = result.foundry.model_deployments.find(
      (d) => d.model_id === result.foundry.active_model && d.status === 'active'
    );
    assert.ok(activeDep, 'Active deployment must exist even in partial config');

    const url = resolveEndpoint(result, activeDep);
    // The base is empty string — resolveEndpoint produces a relative path, not a real URL
    assert.ok(
      !url.startsWith('https://') && !url.startsWith('http://'),
      `SAFETY SMELL: resolveEndpoint produces a non-absolute URL "${url}" when endpoint is missing. ` +
        'Phase 1 should fail-closed at loadFoundryConfig() to prevent this.'
    );
  });
});
