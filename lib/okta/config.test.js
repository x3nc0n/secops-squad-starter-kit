 ...
 * Covers: loadMigrationProfile with committed fixture (valid-profile.yaml),
 * every invalid-enum combination, missing required sections,
 * per_attribute_authority edge cases, and contract (never throws).
 * Operator-file validation is guarded by existsSync — see Section 1b.
 *
 * Fixed in Issue #10: removed hard-dependency on personal .secops/identity/okta.yaml
 * which caused 7 failures on fresh clones. Default suite now uses committed fixtures.
 *
...
// ============================================================================
// 1. loadMigrationProfile — committed fixture (valid-profile.yaml)
// ============================================================================
// Fixed in Issue #10: previously hard-wired to personal .secops/identity/okta.yaml,
// which caused 7 failures on fresh clones. Now uses fixtures/valid-profile.yaml.

describe('config — loadMigrationProfile — committed fixture (valid-profile.yaml)', () => {
  const FIXTURE_PATH = resolve(FIXTURES, 'valid-profile.yaml');

  it('loads and validates fixtures/valid-profile.yaml → {ok:true}', () => {
    const result = loadMigrationProfile(FIXTURE_PATH);
    assert.equal(result.ok, true, `Expected ok:true; got error: ${result.error}`);
    assert.ok(result.data, 'data should be present');
  });

  it('fixture profile has a valid identity_authority enum value', () => {
    const { data } = loadMigrationProfile(FIXTURE_PATH);
    assert.ok(
  ...
  it('fixture profile has all 5 per_class_ownership keys with valid values', () => {
    const { data } = loadMigrationProfile(FIXTURE_PATH);
  ...
  it('fixture profile per_attribute_authority: absent (optional field omission is valid)', () => {
    const { data } = loadMigrationProfile(FIXTURE_PATH);
    const paa = data.source_of_truth.per_attribute_authority;
    if (paa !== undefined) {
      assert.ok(Array.isArray(paa), 'per_attribute_authority must be an array if present');
    }
    // undefined means the field is absent — acceptable for this optional field
  });

  it('fixture profile has a valid strategy block', () => {
    const { data } = loadMigrationProfile(FIXTURE_PATH);
  ...
  it('fixture profile execution.dry_run is a boolean', () => {
    const { data } = loadMigrationProfile(FIXTURE_PATH);
    assert.equal(typeof data.execution.dry_run, 'boolean');
  });

  it('fixture profile execution.entra_write_tooling is a valid enum', () => {
    const { data } = loadMigrationProfile(FIXTURE_PATH);
    assert.ok(
...
// ============================================================================
// 1b. loadMigrationProfile — operator file (guarded, optional)
//
// These tests run ONLY when the operator's personal config exists on disk:
//   .secops/identity/okta.yaml  (gitignored, never committed)
//
// Intent: operators who have the file get it validated locally on every test
// run. Fresh clones and CI (where the file is absent) skip gracefully without
// any failure. Do NOT remove the guard — see Issue #10.
// ============================================================================

describe('config — loadMigrationProfile — operator file (guarded, optional — Issue #10)', () => {
  const operatorFileExists = existsSync(REAL_OKTA_YAML);

  (operatorFileExists ? it : it.skip)('operator file loads with ok:true', () => {
    const result = loadMigrationProfile(REAL_OKTA_YAML);
    assert.equal(result.ok, true, `Expected ok:true; got error: ${result.error}`);
    assert.ok(result.data, 'data should be present');
  });

  (operatorFileExists ? it : it.skip)('operator file has a valid identity_authority enum value', () => {
  ...
  (operatorFileExists ? it : it.skip)('operator file has all 5 per_class_ownership keys with valid values', () => {
    const { data } = loadMigrationProfile(REAL_OKTA_YAML);
    for (const cls of OWNERSHIP_CLASSES) {
      assert.ok(
        VALID.ownership.has(data.source_of_truth.per_class_ownership[cls]),
  ...
  (operatorFileExists ? it : it.skip)('operator file per_attribute_authority is valid if present', () => {
    const { data } = loadMigrationProfile(REAL_OKTA_YAML);
    const paa = data.source_of_truth.per_attribute_authority;
    if (paa !== undefined) {
      assert.ok(Array.isArray(paa), 'per_attribute_authority must be an array if present');
  ...
  (operatorFileExists ? it : it.skip)('operator file has a valid strategy block', () => {
    const { data } = loadMigrationProfile(REAL_OKTA_YAML);
    const s = data.strategy;
    assert.ok(VALID.cutover_shape.has(s.cutover_shape), `cutover_shape: ${s.cutover_shape}`);
    assert.ok(VALID.group_strategy.has(s.group_strategy), `group_strategy: ${s.group_strategy}`);
  ...
  (operatorFileExists ? it : it.skip)('operator file execution.dry_run is a boolean', () => {
    const { data } = loadMigrationProfile(REAL_OKTA_YAML);
    assert.equal(typeof data.execution.dry_run, 'boolean');
  });

  (operatorFileExists ? it : it.skip)('operator file execution.entra_write_tooling is a valid enum', () => {
    const { data } = loadMigrationProfile(REAL_OKTA_YAML);
    assert.ok(
      VALID.entra_write_tooling.has(data.execution.entra_write_tooling),
      `Got: ${data.execution.entra_write_tooling}`
