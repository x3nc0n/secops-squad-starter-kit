...
describe('gate — isDryRun', () => {
  ...
  it('uses real okta.yaml dry_run value — should be true (default-safe default)', () => {
    const r = loadMigrationProfile(REAL_OKTA_YAML);
    assert.equal(r.ok, true);
    assert.equal(isDryRun(r.data), true, 'Real okta.yaml has dry_run: true by design');
  });
});
...
describe('gate — integration: loadMigrationProfile → gate chain', () => {
  it('real okta.yaml: assertEntraOwned blocks ALL 5 classes (all declared okta-owned)', () => {
    const r = loadMigrationProfile(REAL_OKTA_YAML);
    assert.equal(r.ok, true);
    for (const cls of OBJECT_CLASSES) {
  ...
  it('real okta.yaml: dryRunGuard short-circuits (dry_run:true in real file)', () => {
    const r = loadMigrationProfile(REAL_OKTA_YAML);
    assert.equal(r.ok, true);
  ...
  it('real okta.yaml: cutoverWorkflow returns a valid declared shape', () => {
    const r = loadMigrationProfile(REAL_OKTA_YAML);
    assert.equal(r.ok, true);
